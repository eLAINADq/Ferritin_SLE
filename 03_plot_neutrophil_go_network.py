...
def score_column(nodes):
    for column in ("neglog10_padj", "neglog10_pval"):
        if column in nodes.columns:
            return column
    raise KeyError("nodes table must include neglog10_padj or neglog10_pval")
...
def normalize_sizes(values, low=260, high=1300):
...
def draw_graph(graph, output_pdf, output_png, seed, k_factor, iterations, label_width):
    n_nodes = max(1, graph.number_of_nodes())
    layout_k = k_factor / math.sqrt(n_nodes)
    pos = nx.spring_layout(graph, seed=seed, weight="weight", k=layout_k, iterations=iterations)
    ...
    edge_widths = [0.5 + 2.4 * graph[u][v].get("weight", 0.0) for u, v in graph.edges()]

    fig, ax = plt.subplots(figsize=(10, 7), dpi=300, constrained_layout=True)
    nx.draw_networkx_edges(graph, pos, ax=ax, width=edge_widths, edge_color="#B0BEC5", alpha=0.65)
    ...
    labels = {node: wrap_label(node, label_width) for node in representatives(graph)}
...
def main():
    ...
    parser.add_argument("--nodes", default="paper_tables/neutrophil_go_network/network_nodes.tsv")
    parser.add_argument("--edges", default="paper_tables/neutrophil_go_network/network_edges.tsv")
    parser.add_argument("--outdir", default="paper_figures/figure3")
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--k-factor", type=float, default=2.0)
    parser.add_argument("--iterations", type=int, default=200)
    parser.add_argument("--label-width", type=int, default=28)
    ...
    draw_graph(
        graph,
        os.path.join(args.outdir, "figure3_neutrophil_go_network.pdf"),
        os.path.join(args.outdir, "figure3_neutrophil_go_network.png"),
        args.seed,
        args.k_factor,
        args.iterations,
        args.label_width,
